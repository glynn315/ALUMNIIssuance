...
include('../session.php');
$query ="SELECT * FROM studentuseraccount WHERE studentStatus = 'GRADUATING'";  
$result = mysqli_query($conn, $query); 

if (isset($_POST["refresh"])) {
    $query ="SELECT * FROM studentuseraccount WHERE studentStatus = '$_POST[selectOption]'";  
    $result = mysqli_query($conn, $query); 
}
?>
<!DOCTYPE html>
                ...
                <form method="POST">
                    <table class="ml-3 mt-3">
                        <tr>
                            ...
                            <td class="pl-4">
                                <button type="submit" class="btn btn-primary" name="refresh">FILTER</button>
                            </td>
                        </tr>
                    </table>
                </form>
                            ...
                            while($row = mysqli_fetch_array($result))  
                                {  
                                    echo "<tr>";   
                                        ...
                                        echo "<td class='align-middle'>";if ($row["studentStatus"] == 'ALUMNI') {
                                            echo "<a href=studentCredential.php?studentNumber=$row[studentCount]><button class='btn btn-primary mr-2'>View Profiling</button></a>";}else{
                                                echo "<a href=promoted.php?studentNumber=$row[studentCount]><button class='btn btn-primary mr-2'><i class='bi bi-arrow-up-circle-fill'></i> PROMOTE</button></a><a href=studentConsultation.php?studentNumber=$row[studentCount]><button class='btn btn-primary mr-2'><i class='bi bi-arrow-up-circle-fill'></i> EVALUATION</button></a>";
                                            }
                                        }"</td>";
                                    echo "</tr>";
